    ...
    def _open_form(self, hid):
        ...
        if not is_new:
            ...
            if row:
                ...
                if a: existing["animal_id"] = f"{a['id']} – {a['name'] or ''}"

        def on_save(data, win):
            if not data.get("date") or not data.get("animal_id"):
                messagebox.showerror("Required","Date and Animal are required.",parent=win); return
            data["animal_id"] = db.resolve_combo(data["animal_id"])
            if not is_new: data["id"] = hid
            try:
                db.save_health(data); win.destroy(); self.load()
